...
function populateTable() {
    ...
    var tableContentTable = '';
    //tableContentTable += '<script type="text/javascript" src="/javascript/popUp.js"></script>';
    ...
    $.getJSON( '/Products/Productlist', function( data ) {
        ...
        $.each(data, function(){
            ...
            tableContentTable += '<td><a href="#" class="linkshowProduct" rel="' + this.Productname + '" title="Show Details">' + this.Productname + '</a></td>';
            tableContentTable += '<td>' + this.productuser + '</td>';
            if (loggedInUser === this.productuser) {
                tableContentTable += '<td><a href="#" class="linkupdateProduct" rel="' + this._id + '">update</a> | ';
                tableContentTable += '<a href="#" class="linkdeleteProduct" rel="' + this._id + '">delete</a></td>';
            } else {
                tableContentTable += '<td><a href="/offers/makeOffer/'+ this._id +'">make an offer</a></td>';
            }
            tableContentTable += '</tr>';
            tableContentTable += '</tr>';
            tableContentTable += '<tr><td colspan="3">';
            tableContentTable += '<strong>Description:</strong> ';
            tableContentTable += this.desc;
            //tableContentTable += '<span id="ProductInfoName"></span>';
            tableContentTable += '<hr>';
            tableContentTable += '</td></tr>';
        });

        // Inject the whole content string into our existing HTML table
        $('#ProductList table tbody').html(tableContentTable);
...
function populateCategory() {
    ...
    $.getJSON( '/Products/Category', function( data ) {
        ...
        // For each item in our JSON, add a table row and cells to the content string
        var i = 0;
        tableContent += '<a href="#" class="CategoryList" rel="All" title="Show Details">All</a>';
        $.each(data, function(){
            tableContent += ' | ';
            tableContent += '<a href="#" class="CategoryList" rel="' + this + '" title="Show Details">' + this + '</a>';
            i ++;
        });
        // Inject the whole content string into our existing HTML table
        $('#category').html(tableContent);

        $('#CatogryInfo').on('click', 'a.CategoryList', populateTableCategory);
    });
};
// Fill table with data
function populateTableCategory() {

    // Empty content string
    var tableContent = '';
    var getCategoryRel = $(this).attr('rel');

    if (getCategoryRel == 'All') {

        populateTable();

    } else {
        // jQuery AJAX call for JSON
        $.getJSON( '/Products/Category/' + getCategoryRel, function( data ) {
            // Stick our Product data array into a Productlist variable in the global object
            ProductListData = data;
            // For each item in our JSON, add a table row and cells to the content string
            $.each(data, function(){
                ...
                tableContent += '<td>' + this.productuser + '</td>';
                if (loggedInUser === this.productuser) {
                    tableContent += '<td><a href="#" class="linkupdateProduct" rel="' + this._id + '">update</a> | ';
                    tableContent += '<a href="#" class="linkdeleteProduct" rel="' + this._id + '">delete</a></td>';
                } else {
                    tableContent += '<td><a href="/offers/makeOffer/'+ this._id +'">make an offer</a></td>';
                }
                tableContent += '</tr>';
                tableContent += '</tr>';
                ...
                //tableContent += '<span id="ProductInfoName"></span>';
                tableContent += '<hr>';
                tableContent += '</td></tr>';
...
function showProductInfo(event) {
    ...
    $('#ProductInfoUsername').text(thisProductObject.productuser);
    $('#ProductInfoDesc').text(thisProductObject.desc);
...
function updateProductPopulate(event) {
    ...
    $('#addProduct fieldset input#inputProductUser').val(thisProductObject.productuser);
